...
class Supervisor extends BaseApprover {
  ...
  override approveRequest(amount: number): void {
    if(amount <=1000){
      console.log('%cAprobando compra del monto menor a mil',COLORS.green);
      return;
    }
    console.log('No puede aprobar este monto pasando al manager');
    super.next(amount);
  }
}
...
class Manager extends BaseApprover {
  ...
  override approveRequest(amount: number): void {
    if(amount<=5000){
      console.log('%cAprobando compra por el Manager', COLORS.yellow);
      return;
    }
    console.log('No se puede aprobar pasando al director');
    super.next(amount);
  }
}
...
class Director extends BaseApprover {
  approveRequest(amount: number): void {
    if (amount >5000) {
      console.log('%cAprobado por el director', COLORS.blue);
      return;
    }
    console.log('%cError nadie puede aprobar esto',COLORS.red);
    super.next(amount);
  }
